...
// Function writes data from specified virtual memory address. Returns false
// on failure.
bool write_lin_mem(BX_CPU_C *pcpu, bx_address laddr, unsigned len, void *buf) {
  unsigned remainsInPage;
  bx_phy_address paddr;
  unsigned read_len;
  bx_bool paddr_valid;
...
next_page:
  ...
  paddr_valid = pcpu->dbg_xlate_linear2phy(laddr, &paddr);
  if (paddr_valid) {
    BX_MEM(0)->writePhysicalPage(pcpu, paddr, read_len, (Bit8u*)buf);
  } else {
    return false;
  }
