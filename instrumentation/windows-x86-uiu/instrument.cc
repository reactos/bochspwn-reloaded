...
void bx_instr_lin_access(unsigned cpu, bx_address lin, bx_address phy,
  ...
  // We only use movs instructions for tant propagation
  if (globals::rep_movs) {
    ...
    // By default, we mark WRITE mov as initialized to avoid false positive
    // reports.
    if (rw == BX_WRITE) {
      taint::set_init_type(lin, len, MEM_INIT);
    }
...
error_found:
  ...
  */
  ...
  if (rw == BX_READ) {
    ...
    access_type = bug_report_t::MEM_RW;
  }
  else {
    access_type = bug_report_t::MEM_WRITE;
  }
...
void bx_instr_before_execution(unsigned cpu, bxInstruction_c *i) {
  ...
  if (opcode == BX_IA_RET_Op32_Iw) /* Handling of RETn opcode (allocators) */ {
    ...
    globals::rep_movs = true;
             ...
             // READ memory
             opcode == BX_IA_MOV_GqEq || opcode == BX_IA_MOV_GbEb || opcode == BX_IA_MOV_GwEw ||
             opcode == BX_IA_MOV_Op32_GdEd || opcode == BX_IA_MOV_SwEw ||
             opcode == BX_IA_MOV_ALOd || opcode == BX_IA_MOV_AXOd || opcode == BX_IA_MOV_EAXOd ||
             opcode == BX_IA_MOV_EAXOq || opcode == BX_IA_MOV_AXOq || opcode == BX_IA_MOV_ALOq ||
             opcode == BX_IA_MOVSX_GdEb || opcode == BX_IA_MOVSX_GdEw || opcode == BX_IA_MOVSX_GwEb ||
             opcode == BX_IA_MOVZX_GdEb || opcode == BX_IA_MOVZX_GdEw || opcode == BX_IA_MOVZX_GwEb ||
             opcode == BX_IA_MOVD_PqEd || opcode == BX_IA_MOVQ_PqQq || opcode == BX_IA_MOVD_VdqEd ||
             opcode == BX_IA_MOVQ_VqWq || opcode == BX_IA_MOVBE_GwMw || opcode == BX_IA_MOVBE_GdMd ||
             opcode == BX_IA_MOVBE_GqMq ||
             // WRITE memory
             opcode == BX_IA_MOV_EqGq || opcode == BX_IA_MOV_EqId || opcode == BX_IA_MOV_OqEAX ||
             opcode == BX_IA_MOV_OqAX || opcode == BX_IA_MOV_OqAL || opcode == BX_IA_MOV_EbIb ||
             opcode == BX_IA_MOV_EdId || opcode == BX_IA_MOV_EwIw || opcode == BX_IA_MOV_EbGb ||
             opcode == BX_IA_MOV_EwGw || opcode == BX_IA_MOV_Op32_EdGd || opcode == BX_IA_MOV_EwSw ||
             opcode == BX_IA_MOV_OdAL || opcode == BX_IA_MOV_OdAX || opcode == BX_IA_MOV_OdEAX ||
             opcode == BX_IA_MOVD_EdPq || opcode == BX_IA_MOVQ_QqPq || opcode == BX_IA_MOVD_EdVd ||
             opcode == BX_IA_MOVQ_WqVq || opcode == BX_IA_MOVBE_MwGw || opcode == BX_IA_MOVBE_MdGd ||
             opcode == BX_IA_MOVBE_MqGq) {
    globals::mov = true;
